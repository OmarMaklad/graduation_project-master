...
class ServiceChatView extends StatefulWidget {
  final String date;
  final String name;
  final int chatId;
  ServiceChatView({this.name,this.date, this.chatId});
  @override
  _ServiceChatViewState createState() => _ServiceChatViewState();
}
...
class _ServiceChatViewState extends State<ServiceChatView> {
  ...
  Future<void> getMessages({bool loading = false})async{
    id = loggedUser.id;
    _chatModel = await ServiceChatController().getMessage(widget.chatId);
    if(loading)
      _isLoading = false;
    rebuild();
  }
