...
class MakerChatView extends StatefulWidget {
  final String date;
  final String name;
  final int chatId;
  MakerChatView({this.name,this.date, this.chatId});
  @override
  _MakerChatViewState createState() => _MakerChatViewState();
}
...
class _MakerChatViewState extends State<MakerChatView> {
  ...
  Future<void> getMessages({bool loading = false})async{
    id = loggedUser.id;
    _chatModel = await MakerChatController().getMessage(widget.chatId);
    if(loading)
      _isLoading = false;
    rebuild();
  }
