...
class SalerChatView extends StatefulWidget {
  final String date;
  final String name;
  final int chatId;
  SalerChatView({this.name,this.date, this.chatId});
  @override
  _SalerChatViewState createState() => _SalerChatViewState();
}

class _SalerChatViewState extends State<SalerChatView> {
  SalerChatModel _chatModel;
  ...
  Future<void> getMessages({bool loading = false})async{
    id = loggedUser.id;
    _chatModel = await SalerChatController().getMessage(widget.chatId);
    if(loading)
      _isLoading = false;
    rebuild();
  }
