...
class BuyerChatView extends StatefulWidget {
  ...
  final String name;
  final int chatId;
  BuyerChatView({this.name,this.date, this.chatId});
  @override
  _BuyerChatViewState createState() => _BuyerChatViewState();
}

class _BuyerChatViewState extends State<BuyerChatView> {
  BuyerChatModel _chatModel;
  ...
  Future<void> getMessages({bool loading = false})async{
    id = loggedUser.id;
    _chatModel = await BuyerChatController().getMessage(widget.chatId);
    if(loading)
      _isLoading = false;
    rebuild();
  }
